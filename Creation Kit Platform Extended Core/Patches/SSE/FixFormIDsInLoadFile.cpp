		...
		namespace SkyrimSpectialEdition
		{
				...
				const RelocationDatabaseItem* lpRelocationDatabaseItem)
			{
				if (lpRelocationDatabaseItem->Version() == 1)
				{
					...
					lpRelocator->DetourCall((uintptr_t)Rva + 8, (uintptr_t)&sub2);
					lpRelocator->DetourCall((uintptr_t)Rva2 + 0xD, (uintptr_t)&sub3);
					lpRelocator->DetourCall((uintptr_t)Rva3 + 5, (uintptr_t)&sub4);
			...
			void FixFormIDsInLoadFilePatch::sub2(uintptr_t Stack)
			{
				// I found a cell form in the stack, I will use it
				auto CellForm = *(TESForm**)(Stack + 0x698);
				auto FormIDs = (uint32_t*)(Stack + 0xF0);

				if ((*FormIDs > 0) && CellForm)
				{
					auto SafeFormIDs = *FormIDs;
					bool Ret = false;
					auto HighIndexLoop = 0;
					// I assume that the location has an high-index of the cell itself.
					auto HighIndex = CellForm->GetFormID() & 0xFF000000;
				...
				Loop:
					...
					NextLoop:
						...
						if (HighIndexLoop <= 0x60)	// Limit master
						{
			...
			uint32_t FixFormIDsInLoadFilePatch::sub3(TESForm* World, uint32_t* FormIDs)
			{
			...
			void FixFormIDsInLoadFilePatch::sub4(uintptr_t Stack)
			{
				auto LocForm = *(TESForm**)(Stack + 0x150);	
				auto FormIDs = (uint32_t*)(Stack + 0x54);
