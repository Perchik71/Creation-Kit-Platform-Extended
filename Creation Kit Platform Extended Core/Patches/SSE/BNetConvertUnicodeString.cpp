		...
		namespace SkyrimSpectialEdition
		{
				...
				const RelocationDatabaseItem* lpRelocationDatabaseItem)
			{
				...
				else if (lpRelocationDatabaseItem->Version() == 2)
				{
					...
					//
					lpRelocator->DetourJump(lpRelocationDatabaseItem->At(0), (uintptr_t)&BNetConvertUnicodeString);
					lpRelocator->DetourJump(lpRelocationDatabaseItem->At(1), (uintptr_t)&BNetConvertUnicodeString);
				...
				const wchar_t* Source, size_t SourceSize)
			{
				...
				int outLen = (size_t)WideCharToMultiByte(CP_UTF8, 0, Source, (int)SourceSize, Destination,
