...
#include "BGSSoundDescriptorForm.h"
...
namespace CreationKitPlatformExtended
{
	namespace EditorAPI
	{
		namespace SkyrimSpectialEdition
		{
			...
			{
			public:
				constexpr static uint8_t TYPE_ID = ftImpactData;
				
				enum OrientationT : uint32_t
				{
					eoSurfaceNormal = 0,
					eoProjectileVector,
			...
			public:
				...
				inline uint8_t GetDecalParallaxPasses() const { return _DecalParallaxPasses; }
				inline void SetDecalParallaxPasses(uint8_t v) { _DecalParallaxPasses = v; }
				...
				inline void SetOwnDecalData(bool v) { _HasOwnDecalData = (uint8_t)(!v); }
				...
				inline void SetDecalColorRGB(uint8_t r, uint8_t g, uint8_t b) { _DecalColor = { r, g, b }; }
			private:
				char pad28[0x8];
				...
				uint8_t _HasOwnDecalData;					// 1 - false, 0 - true (◕‿◕)
