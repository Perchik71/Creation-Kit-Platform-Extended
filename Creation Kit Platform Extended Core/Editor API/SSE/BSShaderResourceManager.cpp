				...
				NiPick& kPick, BSTriShape* pkTriShape)
			{
				...
				if (indices && vertexData[0])
				{
					...
					for (uint32_t i = 0; i < (pkTriShape->m_TriangleCount * 3u); i += 3)
					{
						...
						{
							auto record = ((NiPick::Record * (__fastcall*)(uintptr_t, BSTriShape*))pointer_PickGetRecord_sub)((uintptr_t)&kPick + 0x20, pkTriShape);
			...
			bool BSShaderResourceManager_CK::FindIntersectionsTriShapeFastPathEx(NiPoint3& kOrigin, const NiPoint3& kDir, 
				...
				if (indices && vertexData[0])
				{
					...
					for (uint32_t i = 0; i < (pkTriShape->m_TriangleCount * 3u); i += 3)
					{
						...
						if (NiCollisionUtils::IntersectTriangle(kModelOrigin, kModelDir, v[0], v[1], v[2], kPick.GetFrontOnly(), intersect, lineParam, triParam1, triParam2))
						{
							auto record = ((NiPick::Record* (__fastcall*)(uintptr_t, BSTriShape*))pointer_PickGetRecord_sub)((uintptr_t)&kPick, pkTriShape);
							intersectionFound = true;
