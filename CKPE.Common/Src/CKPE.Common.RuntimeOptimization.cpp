...
namespace CKPE
{
	namespace Common
	{
		std::uint64_t RuntimeOptimization::RemoveTrampolines(std::uintptr_t target, std::uintptr_t size)
		{
			return 0;
		}
		...
		void RuntimeOptimization::Apply()
		{
			...
			tasks.push_back(RemoveTrampolines(seg_code.GetAddress(), seg_code.GetSize()));
			...
			_MESSAGE("%s: %llu patches applied in %llums.", __FUNCTION__, total, duration);

			//auto beg = seg_code.GetPointer<std::uint8_t>();
