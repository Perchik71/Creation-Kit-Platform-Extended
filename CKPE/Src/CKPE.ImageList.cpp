...
namespace CKPE
{
	...
	std::int32_t ImageList::AddFromResource(THandle hinst, std::uint32_t dwResId, const wchar_t* name,
	...
	std::int32_t ImageList::AddFromResource(THandle hinst, std::uint32_t dwResId, std::uint32_t dwResMaskId) noexcept(true)
	{
		if (!IsCreated()) return -1;

		Bitmap bm(hinst, dwResId);
		Bitmap bmmask;

		if (dwResMaskId)
			bmmask.LoadFromResource(hinst, dwResMaskId);

		return (!bm.Empty()) ? (std::int32_t)ImageList_Add((HIMAGELIST)m_hHandle, (HBITMAP)bm.Handle,
			(HBITMAP)bmmask.Handle) : -1;
	}

	std::int32_t ImageList::AddFromResourceMasked(THandle hinst, std::uint32_t dwResId, const wchar_t* name,
	...
	std::int32_t ImageList::AddFromResourceMasked(THandle hinst, std::uint32_t dwResId, TColor cmask) noexcept(true)
	{
		if (!IsCreated()) return -1;
	...
	bool ImageList::ReplaceFromResource(std::int32_t idx, THandle hinst, std::uint32_t dwResId,
		const wchar_t* name, std::uint32_t dwResMaskId) noexcept(true)
	{
		if (!IsCreated()) return false;
