...
namespace CKPE
{
	namespace Fallout4
	{
		namespace Patch
		{
			class PatchMainWindow : public Common::Patch, public Common::UI::CUIMainWindow
			{
			protected:
				WNDPROC _oldWndProc{ nullptr };
			public:
				PatchMainWindow() noexcept(true) = default;

				[[nodiscard]] inline WNDPROC GetOldWndProc() const noexcept(true) { return _oldWndProc; }
			};

			class MainWindow : public PatchMainWindow
			{
				static void DoOpenFormByIdHandler(std::uint32_t id) noexcept(true);
				static void FogToggling() noexcept(true);
				static void MarkerToggling() noexcept(true);
