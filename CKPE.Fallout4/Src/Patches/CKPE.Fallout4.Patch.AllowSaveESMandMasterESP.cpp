		...
		namespace Patch
		{
			...
			bool AllowSaveESMandMasterESP::DoActive(Common::RelocatorDB::PatchDB* db) noexcept(true)
			{
				...
				else 
				{
					...
					EditorAPI::pointer_TESFile_sub1 = addressLibrary->Resolve(122342);
					EditorAPI::pointer_TESFile_sub2 = addressLibrary->Resolve(122343);
					pointer_AllowSaveESMandMasterESP_sub1 = addressLibrary->Resolve(117094);
					...
					{
						*(std::uintptr_t*)&EditorAPI::TESFile::LoadTESInfo =
							Detours::DetourClassJump(addressLibrary->Resolve(122245), &EditorAPI::TESFile::hk_LoadTESInfo);
						*(std::uintptr_t*)&EditorAPI::TESFile::WriteTESInfo =
							Detours::DetourClassJump(addressLibrary->Resolve(122248), &EditorAPI::TESFile::hk_WriteTESInfo);

						if (EditorAPI::TESFile::AllowSaveESM)
						{
							// Also allow non-game ESMs to be set as "Active File"
							Detours::DetourClassCall(addressLibrary->Resolve(115777) + 0x5F, &IsActiveFileBlacklist);
							SafeWrite::WriteNop(addressLibrary->Resolve(122023) + 0x4F, 2);

							// Disable: "File '%s' is a master file or is in use.\n\nPlease select another file to save to."
							const char* newFormat = "File '%s' is in use.\n\nPlease select another file to save to.";

							SafeWrite::WriteNop(addressLibrary->Resolve(122036) + 0x55A, 13);
							SafeWrite::Write(addressLibrary->Resolve(760858), (std::uint8_t*)newFormat, strlen(newFormat) + 1);
							Detours::DetourJump(addressLibrary->Resolve(117100), (std::uintptr_t)&OpenPluginSaveDialog);
						}

						if (EditorAPI::TESFile::AllowMasterESP)
							// Remove the check for IsMaster()
							SafeWrite::WriteNop(addressLibrary->Resolve(122092) + 0x5C, 9);
