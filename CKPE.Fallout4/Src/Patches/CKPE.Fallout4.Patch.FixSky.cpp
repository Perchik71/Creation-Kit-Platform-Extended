	...
	namespace Fallout4
	{
		namespace Patch
		{
			...
			bool FixSky::DoActive(Common::RelocatorDB::PatchDB* db) noexcept(true)
			{
				...
				const auto target = ID(535695);

				if (VersionLists::GetEditorVersion() == VersionLists::EDITOR_FALLOUT_C4_1_10_162_0)
				{
					Relocation(target, 0x1D1).Write({ 0xEB, 0x4D, 0x90 });
					Relocation(target, 0x220).Write({ 0x48, 0x85, 0xC9, 0x74, 0xB5, 0x48, 0x8B, 0x01, 0xEB, 0xAA });
				}
				else
				{
					Relocation(target, 0x1EF).Write({ 0xEB, 0x54, 0x90 });
					Relocation(target, 0x245).Write({ 0x48, 0x85, 0xC9, 0x74, 0xAE, 0x48, 0x8B, 0x01, 0xEB, 0xA3 });
				}
