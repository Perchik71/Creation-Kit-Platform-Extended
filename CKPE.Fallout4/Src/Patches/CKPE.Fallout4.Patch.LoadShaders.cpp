...
namespace CKPE
{
	namespace Fallout4
	{
		namespace Patch
		{
			...
			class ResourceArchiveShader : public EditorAPI::BSResource::Archive2::ReaderStream
			{
			public:
			...
			bool LoadShaders::DoActive(Common::RelocatorDB::PatchDB* db) noexcept(true)
			{
				...
				EditorAPI::BSResource::Archive2::ReaderStream::RTTI = 
					Common::RTTI::GetSingleton()->Find("class BSResource::Archive2::ReaderStream")->VTableAddress;

				Detours::DetourCall(__CKPE_OFFSET(0) + ((verPatch == 1) ? 0x88 : 0x8C), (std::uintptr_t)&CreateStream);
				pointer_LoadShaders_sub = __CKPE_OFFSET(1);

				return true;
			}

			EditorAPI::BSIStream* LoadShaders::CreateStream(EditorAPI::BSIStream* Stream, const char* FileName,
				std::uint32_t unk1, bool unk2) noexcept(true)
			{
				auto streamShaders = fast_call<EditorAPI::BSIStream*>(pointer_LoadShaders_sub, Stream, FileName, unk1, unk2);	
				streamShaders->NewStream(new ResourceArchiveShader());
