	...
	namespace Fallout4
	{
		namespace Patch
		{
			...
			bool FixLandspaceVNML::DoActive(Common::RelocatorDB::PatchDB* db) noexcept(true)
			{
				...
				auto rel = Relocation(ID(288239), 0x1B0);
				rel.WriteFill(NOP, 0x12B);
				...
				public:
					...
					static VOID Generate(std::uintptr_t Target)
					{
						...
						Detours::DetourJump(Target, reinterpret_cast<std::uintptr_t>(hook->getCode()));
					}
				};

				xNormalize_Hook::Generate(Rva1);
				Relocation(ID(1621802)).WriteJump(&EditorAPI::Forms::TESObjectLAND::Layers::HKNormalize);
