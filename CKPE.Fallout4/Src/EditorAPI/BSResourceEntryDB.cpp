...
#include <EditorAPI/BSResourceEntryDB.h>
	...
	namespace Fallout4
	{
		namespace EditorAPI
		{
			namespace BSResource
			{
				...
				void ReleaseHashDB(BSHashDB* Hash) noexcept(true)
				{
				...
				bool BSTextureDB::Exists(const char* Path) noexcept(true)
				{
					if (!Path || Path[0] == '\0')
						return false;
...

					{
						...
						//if (!Result && (*(std::uint8_t**)Cache))
						//	// If it is not found, try to find it in the cache.
						//	Result = (bool)GetTextureDBTraitsFromCache((*(std::uint8_t**)Cache) + 0x100, &Hash, &Traits);						
						// Relese hash
						ReleaseHashDB(&Hash);
					}
