...
namespace CKPE
{
	namespace SkyrimSE
	{
		namespace EditorAPI
		{
			...
			static std::uintptr_t pointer_Renderer = 0;
			static std::uintptr_t pointer_D3D11Device = 0;

			template<std::uint32_t RenderTargetCount, std::uint32_t DepthStencilsCount,
				std::uint32_t CubemapRenderTargetCount>
			class BSRenderer
			{
			public:
				char _pad0[0xAF0];
					...
					const DepthStencilTargetProperties* Properties)
				{
					...
					for (std::uint32_t i = 0; i < Properties->uiArraySize; i++)
					{
				...
				void DestroyRenderTarget(std::uint32_t TargetIndex)
				{
					auto data = &pRenderTargets[TargetIndex];
				...
				void DestroyDepthStencil(std::uint32_t TargetIndex)
				{
					auto data = &pDepthStencils[TargetIndex];
				...
				void DestroyCubemapRenderTarget(std::uint32_t TargetIndex)
				{
					auto data = &pCubemapRenderTargets[TargetIndex];
			...
			template<std::uint32_t RenderTargetCount, std::uint32_t DepthStencilCount,
				std::uint32_t CubemapRenderTargetCount, const char** RenderTargetNames,
