		...
		namespace EditorAPI
		{
			namespace BSGraphics
			{
				namespace BSShaderRenderTargets
				{
					...
					const char* GetRenderTargetName_1130(std::uint32_t Index);
					const char* GetCubemapName(std::uint32_t Index);
					const char* GetTexture3DName(std::uint32_t Index);
				...
				enum ClearDepthStencilTarget : std::uint32_t
				{
				...
				enum DepthStencilStencilMode : std::uint32_t
				{
				...
				enum DepthStencilDepthMode : std::uint32_t
				{
				...
				enum RasterStateCullMode : std::uint32_t
				{
				...
				//
				const std::uint8_t INVALID_CONSTANT_BUFFER_OFFSET = 0xFF;
...
#pragma warning(disable:94)   // Intel C++ Compiler
				struct VertexShader
				{
					...
					std::uint32_t m_ShaderLength;				// Raw bytecode length
					...
					std::uint64_t m_VertexDescription;					// ID3D11Device::CreateInputLayout lookup (for VSMain)
					std::uint8_t m_ConstantOffsets[MAX_VS_CONSTANTS];	// Actual offset is multiplied by 4
					std::uint8_t __padding[4];
					std::uint8_t m_RawBytecode[0];						// Raw bytecode
				};
				...
				struct ComputeShader
				{
					...
					std::uint32_t m_TechniqueID;						// Bit flags
					std::uint32_t m_ShaderLength;						// Raw bytecode length
					std::uint8_t m_ConstantOffsets[MAX_CS_CONSTANTS];	// Actual offset is multiplied by 4
					std::uint8_t m_RawBytecode[0];						// Raw bytecode
				};
...
#pragma warning(pop)
				...
				struct DynamicTriShape
				{
					...
					ID3D11Buffer* m_IndexBuffer;
					std::uint64_t m_VertexDesc;
					std::uint32_t m_VertexAllocationOffset;
					std::uint32_t m_VertexAllocationSize;
					std::uint32_t m_RefCount;
					void* m_RawVertexData;
					void* m_RawIndexData;
				};
