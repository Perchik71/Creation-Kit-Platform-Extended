	...
	namespace SkyrimSE
	{
		namespace EditorAPI
		{
			namespace BSGraphics
			{
				...
				{
					const char* Debug_RenderTargetNames[RENDER_TARGET_COUNT] =
					{
					...
					const char* Debug_RenderTargetNames_1130[RENDER_TARGET_COUNT_1130] =
					{
						"RENDER_TARGET_FRAMEBUFFER",
						"RENDER_TARGET_MAIN",
						"RENDER_TARGET_MAIN_COPY",
						...
						"RENDER_TARGET_SNOW_SPECALPHA",
						"RENDER_TARGET_SNOW_SWAP",
						"RENDER_TARGET_UNKNOWN_1",
						"RENDER_TARGET_UNKNOWN_2",
					};

					const char* Debug_CubemapNames[RENDER_TARGET_CUBEMAP_COUNT] =
					{
						"RENDER_TARGET_CUBEMAP_REFLECTIONS",
					};

					const char* Debug_3DTextureNames[TEXTURE3D_COUNT] =
					{
					...
					const char* Debug_DepthStencilNames[DEPTH_STENCIL_COUNT] =
					{
					...
					const char* GetRenderTargetName(std::uint32_t Index)
					{
						...
						CKPE_ASSERT(Index < RENDER_TARGET_COUNT);
						return Debug_RenderTargetNames[Index];
					}

					const char* GetRenderTargetName_1130(std::uint32_t Index)
					{
						if (Index == RENDER_TARGET_COUNT_1130)
							return "RENDER_TARGET_NONE";

						CKPE_ASSERT(Index < RENDER_TARGET_COUNT_1130);
						return Debug_RenderTargetNames_1130[Index];
					}
					...
					const char* GetCubemapName(std::uint32_t Index)
					{
						...
						CKPE_ASSERT(Index < RENDER_TARGET_CUBEMAP_COUNT);
					...
					const char* GetTexture3DName(std::uint32_t Index)
					{
						...
						CKPE_ASSERT(Index < TEXTURE3D_COUNT);
					...
					const char* GetDepthStencilName(std::uint32_t Index)
					{
						...
						CKPE_ASSERT(Index < DEPTH_STENCIL_COUNT);
